...
function typeAhead(search) {
  ...
  searchInput.on("input", function () {
    ...
    axios.get("/api/search?q=" + this.value).then(function (res) {
      ...
      // Tell them nothing in the DB with that name
  ...
  // handlekewboard inputs
  searchInput.on("keyup", function (e) {
    //if they aren't pressing up, down or enter, nothing
...
function loadPlaces(map) {
  ...
  _axios2.default.get("/api/stores/near?lat=" + lat + "&lng=" + lng).then(function (res) {
    ...
    var markers = places.map(function (place) {
      ...
      var marker = new google.map.Marker({ map: map, position: position });
      marker.place = place;
      return marker;
    });
